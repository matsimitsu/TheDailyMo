...
rotate:function(parameters)
{
	...
	for (var i=0,i0=this.length;i<i0;i++)
	{
		...
		{
			element.Wilq32.PhotoEffect._rotate(parameters.angle, parameters.scale);
		}
	}
	return returned;
},
...
(function()
{
	...
	return function()
	{
		...
		this._img._width=width   // placeholders
		this._img._height=height // placeholders
		...
		this._rotate(this._parameters.angle, this._parameters.scale);
...
{
	if (jQuery.browser.msie)
	return function(angle,scale)
	{
		var width=this._img._width;
		var height=this._img._height;
		var upper_corner = height/2 - parseFloat(height)*scale/2
		var left_corner  = width/2 - parseFloat(width)*scale/2
		this._vimage.style.height=(parseFloat(height)*scale)+"px";
		this._vimage.style.width =(parseFloat(width)*scale)+"px";
		this._vimage.style.top  = (((this._img._heightMax-height)/2) + upper_corner)+"px";
		this._vimage.style.left = (((this._img._widthMax-width)/2) +   left_corner)+"px";
...

	{
		...
		// this._cnv.scale(scale, scale); // SCALE - if needed ;)
